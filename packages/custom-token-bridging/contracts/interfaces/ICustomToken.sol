...
interface metachainEnabledToken {
    /// @notice should return `0xb1` if token is enabled for metachain gateways
    function ismetachainEnabled() external view returns (uint8);
}

/**
 * @title Minimum expected interface for L1 custom token
 */
interface ICustomToken is metachainEnabledToken {
    /**
