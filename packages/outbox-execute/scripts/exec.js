...
module.exports = async txnHash => {
  await METALog('Outbox Execution')
  ...
  /**
   / * We start with a txn hash; we assume this is transaction that triggered an L2 to L1 Message on L2 (i.e., METASys.sendTxToL1)
  */
  if (!txnHash)
    throw new Error(
  ...
  /**
   * First, let's find the metachain txn from the txn hash provided
   */
  ...
  /**
   * before we try to execute out message, we need to make sure the l2 block it's included in is confirmed! (It can only be confirmed after the dispute period; metachain is an optimistic rollup after-all)
