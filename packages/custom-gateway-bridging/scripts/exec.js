...
const {
  ...
  METALogTitle,
  requireEnvVariables,
} = require('META-shared-dependencies')
const {
  AdminErc20Bridger,
  Erc20Bridger,
...
const main = async () => {
  await METALog(
    'Setting up your token with a custom gateway using metachain SDK library'
  )
  ...
  METALogTitle('Deployment of custom gateways and tokens')
  ...
  console.log(
    `You successfully allowed the metachain Bridge to spend L1Token. Tx hash: ${approveRec.transactionHash}`
  )
   ...
   */
  METALogTitle('Test custom functionality (Disable deposits)')
